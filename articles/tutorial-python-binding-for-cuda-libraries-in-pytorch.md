...
现在我们有了关于它们的输入和信息，让我们检查它们是否是有效的输入。通过访问张量形状和数据类型，一些更琐碎的检查（例如，矩阵乘法的兼容维度）应该是不言自明的。因此，我们将重点关注 Torch 和 CUTLASS 更具体的主题。
...
现在我们有了函数，让我们编译它并将其绑定到 Python 函数。我们将使用`PyBind11`结合`setuptools`来执行这一步。我们不会全面讨论这些工具，而只会讨论与我们直接相关的内容。
