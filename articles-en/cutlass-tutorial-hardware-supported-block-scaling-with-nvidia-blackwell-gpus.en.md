...
external: false
chinese_markdown: "articles/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus.md"
---
# CUTLASS Tutorial: Hardware-supported Block-scaling with NVIDIA Blackwell GPUs

中文翻译：[articles/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus.md](../articles/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus.md)
...
![Figure 1. Blockscaled GEMM, from the PTX documentation .](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-4-c1fc0073a7.png)
...
![Figure 2. Packing of 4-bit values in SMEM, from the PTX documentation .](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-5-bc3655281b.png)
...
![Figure 3. TMEM scale factor layout for .scale_vec::1X , from the PTX documentation .](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-11-e29061b2fe.png)
...
![Figure 4. TMEM scale factor layout for .scale_vec::2X , from the PTX documentation .](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-12-61c2f47fce.png)
...
![Figure 5. TMEM scale factor layout for .scale_vec::4X , from the PTX documentation .](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-10-bd60931167.png)
...
![Figure 6. GMEM layout for a tile of SFA in the interleaved layout. Note that this entire tile should be contiguous in GMEM. From the CUTLASS documentation .](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-3-1d148f13b6.png)

This 512B tile would then be tiled over the entire SFA tensor:

![Figure 7. GMEM layout for SFA, created by tiling the basic tile of Figure 6 over all of SFA. From the CUTLASS documentation .](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-6-06a28071bf.png)
...
- The `sfa_smem_layout_staged`’s layout matches the TMEM diagram of [Figure 4](https://research.colfax-intl.com/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/#figure-4): 32 : 0 corresponds to sf_vec_K — a single SFA element being applied to 32 elements of A in the K direction  In this case, 32 is also the K extent of an MMA atom.  4 : 1 corresponds to MMA_K — 4 contiguous SFA elements are used for 4 separate MMA atoms repeated in the K direction  (32, 4) : (16, 4) corresponds to sf_tile_M — 32 scale factor rows correspond to 32 MMA A rows (strided by 1 row of a tile, i.e. 16 values, in GMEM and SMEM), then the next 32 MMA A rows are repeated 4 columns later in the scale factor layout, and so on.
...
If we were instead doing `nvf4` GEMM (corresponding to the `.block16/.scale_vec::4X` TMEM layout of [Figure 5](https://research.colfax-intl.com/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/#figure-5)), the scale factor tiles would look like this:
...
![Figure 8. TMEM layout for mxf8 GEMM with 128×256 tile size.](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-3ef067cdac.png)
...
![Figure 9. TMEM layout for nvf4 GEMM with 128×256 tile size.](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-1-0bd4082d1e.png)
...
![Figure 10. The TMA loading pattern for SFB when bN = 192. Each work tile with even N coordinate and the following odd work tile both load the middle SFB tile, but only use half of it.](../images/cutlass-tutorial-hardware-supported-block-scaling-with-nvidia-blackwell-gpus/image-2-132cc88e6f.png)
